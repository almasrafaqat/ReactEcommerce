          ...
          if(isColor==="YES" && color.length===0){
               ...
               axios.post(AppURL.addToCart, MyFormData).then(response=>{
                    if(response.data===1){
                         ...
                         cogoToast.error("ooops Product Not Added ! Try Aagain",{position:'top-right'});
                         this.setState({addToCart:"Add To Cart"})
                    }
               })
               .catch(error=>{
     ...
     addToFav = () => {
          ...
          if(!localStorage.getItem('token')){
               ...
               axios.get(AppURL.AddFavourite(productCode,email)).then(response=>{
                    if(response.data===1){
                         cogoToast.success("Product Is in Favourite",{position:'top-right'});
                         this.setState({addToFav:"Favourite"})
...
<Col className="shadow-sm bg-white pb-3 mt-4" md={12} lg={12} sm={12} xs={12}>
     <Row>
          ...
          <InnerImageZoom src={this.state.previewImg} zoomSrc={this.state.previewImg} className="detailimage" zoomScale={1.8} zoomType={"hover"} />
          <Container  className="my-3">
               <Row>
               <Col className="p-0 m-0"  md={3} lg={3} sm={3} xs={3}>
                         <img onClick={this.imgOnClick} className="product-sm-img smallimage" src={image} />
                    </Col>
                    <Col className="p-0 m-0"  md={3} lg={3} sm={3} xs={3}>
                         <img onClick={this.imgOnClick} className="product-sm-img smallimage" src={image_one} />
                    </Col>
                    <Col className="p-0 m-0" md={3} lg={3} sm={3} xs={3}>
                         <img onClick={this.imgOnClick} className="product-sm-img smallimage" src={image_two} />
                    </Col>
                    <Col className="p-0 m-0" md={3} lg={3} sm={3} xs={3}>
                         <img onClick={this.imgOnClick} className="product-sm-img smallimage" src={image_three} />
                    </Col>
                    <Col className="p-0 m-0" md={3} lg={3} sm={3} xs={3}>
                         <img onClick={this.imgOnClick} className="product-sm-img smallimage" src={image_four} />
                    </Col>
               </Row>
          </Container>
          </Col>
